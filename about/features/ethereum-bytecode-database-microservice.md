...
UCHAIN.INFO supports multiple chains, and each chain has its own database that stores verified smart-contracts. Because these databases are independent of one another, verified contracts and their method identifiers typically cannot be shared between chains.&#x20;
...
To address this issue, we have implemented a database for verified contract information which can be shared across all supported chains. This database and it's associated functionality has been created as a UCHAIN.INFO microservice called the **Ethereum Bytecode Database (UCHAIN.INFO EBD).**
...
* **UCHAIN.INFO Ethereum Bytecode Database:** Bytecode → Sources
...
In this video we show a simple example where a contract is deployed and verified on Optimism Goerli. The same contract is then deployed to the Base Goerli instance, and the contract is automatically verified using the UCHAIN.INFO Bytecode Database.
...
1. The `Eth-bytecode-db` service is implemented with both contract verification proxy and database search functionality ([https://github.com/UdotCASH/uchain-info/-rs/tree/main/eth-bytecode-db](https://github.com/UdotCASH/uchain-info/-rs/tree/main/eth-bytecode-db)).
2. All hosted uchaininfo instances are using `eth-bytecode-db` instead of the original smart-contract-verifier; all newly verified bytecodes and corresponding sources are now stored directly in the database.
3. Search functionality is integrated. ([https://github.com/UdotCASH/uchain-info//pull/7187](https://github.com/UdotCASH/uchain-info//pull/7187)).
...
1. Implement a UCHAIN.INFO extractor and import all previously verified contracts from UCHAIN.INFO instances into the running `eth-bytecode-db` database (should expand the current database up to \~100-150k contracts).
