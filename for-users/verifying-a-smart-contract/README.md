...
Verification is available for both Solidity and Vyper contracts. **Currently, there are 7 methods for verification using the UCHAIN.INFO UI.**
...
<figure><img src="../../.gitbook/assets/sourcify-uchaininfo-1.png" alt=""><figcaption></figcaption></figure>
...
10. If all goes well, you will see a checkmark :white\_check\_mark: next to Code in the code tab, and an additional tab called `Read Contract`. The contract name will now appear in uchaininfo with any transactions related to your contract.
...
If all goes well, you will see a checkmark :white\_check\_mark: next to Code in the code tab, and an additional tab called `Read Contract`. The contract name will now appear in uchaininfo with any transactions related to your contract.
...
The [Hardhat verification plugin](hardhat-verification-plugin.md) supports uchaininfo. You can also choose to use the [Sourcify plugin](sourcify-plugin-for-hardhat.md) to verify with Sourcify from your hardhat environment. [Foundry supports uchaininfo verification with Forge](https://book.getfoundry.sh/reference/forge/forge-verify-contract).
