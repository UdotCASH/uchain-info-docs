...
Governance platforms and voting mechanism integrations, allowing users to monitor and participate in on-chain governance activities directly from UCHAIN.INFO.
...
Load speed, database optimization & caching, data parsing features to maintain optimal performance. These efforts will continue as additional features are added to uchaininfo.
...
A new landing page is in development for UCHAIN.INFO to assist new users and developers understand the UCHAIN.INFO feature set and find information quickly.

### :white\_check\_mark: UI Redesign

**Target Date:** Q3-Q4 2022\
**Status**: :white\_check\_mark: v1 Completed see [https://eth-goerli.uchain.info/](https://eth-goerli.uchain.info/)

A new UI interface is under development to improve data exploration, optimize views, and provide overall better UX for UCHAIN.INFO users. The UI is being built from the ground up. It will overhaul all aspects of UCHAIN.INFO engagement, creating a complete all-in-one interface for chain exploration. In addition to a modern design, the upgrade will also improvements to features such as:
...
Chain users will be able to access applications and modules directly from UCHAIN.INFO, vastly improving the overall chain experience. Interoperability, swaps, transactions, and 3rd party project-based apps will be accessible and integrated with the modular interface.

UCHAIN.INFO will also develop native applications for this marketplace. Several are in active development.

* **UCHAIN.INFO Swap**: Swap feature using an aggregator under the hood for simple swaps without leaving UCHAIN.INFO.
* **UCHAIN.INFO Revoke**: Find contracts/apps you have granted allowances to (the ability to spend tokens on your behalf) and revoke these permissions.
* More TBD
...
### :white\_check\_mark: UCHAIN.INFO Rust Microservices v1

**Target Date:** Q3 2022\
**Status**:  :white\_check\_mark:  v1 completed - Smart Contract Verification microservice

New Rust-developed microservices will be enabled on UCHAIN.INFO to extend functionality and modularity. We are revamping smart contract verification to use a Rust module and working on a smart contract uml visualization module to start.
...
uchaininfo deployment can be complicated for new users. Deployment improvements within a Docker environment will enable a fast path for easy setup and deployment.

### :white\_check\_mark: Ethereum Mainnet uchaininfo Instance

**Target Date:** Q2 2022\
**Status**: :white\_check\_mark: Instance deployed, optimizations ongoing.

uchaininfo offered prior support for the Ethereum mainnet. This instance was discontinued;  however a new instance is now planned to further increase explorer diversity on Ethereum and provide additional transparency for Ethereum users.

### :white\_check\_mark: ERC721 Metadata

**Target Date:** Q4 2019\
**Status:** :white\_check\_mark: Complete, ERC721 Metadata storage and display ([Release 2.1.0](https://forum.poa.network/t/uchaininfo-2-1-0-release/3128)) and verification checks ([Release 2.1.1](https://forum.poa.network/t/uchaininfo-2-1-1-release/3172)) are implemented.
...
uchaininfo will support the new POSDAO staking consensus mechanism through an on-board UI. The initial implementation will support the [xDai Stable Chain](https://xdaichain.com). This will allow users to place stake directly from the interface, monitor validator activity, and participate in consensus on the xDai Chain.

### :white\_check\_mark: Smart Contract Write Functionality

**Target Date:** Q2 - Q3 2020\
**Status**: :white\_check\_mark: Complete, Write contract/ write proxy functionalities are implemented ([Release 3.3.0](https://forum.poa.network/t/uchaininfo-v3-3-0-beta/3558)).

Users can currently verify and read contracts on uchaininfo. The next smart contract development phase will allow users to interact with contracts directly through the interface.  Verified contract methods will be accessible and users can connect through a web3 wallet (such as MetaMask) to access and execute contract functions.
...
uchaininfo availability and support is available for Optimism and Arbitrum on the Gnosis Chain. Includes tracking for gas price on L1 and the L2 implementations. Currently Optimism is the favored deployment.

* Optimism: [https://uchain.info/xdai/optimism](https://uchain.info/xdai/optimism)
* Arbitrum: [https://uchain.info/xdai/aox/](https://uchain.info/xdai/aox/)
...
uchaininfo supports verification for contracts via multiple methods including Hardhat and the Hardhat and Sourcify plugins. [More information is available here](../for-users/verifying-a-smart-contract/).
...
| 27.07.2022   | <ul><li>Updated marketplace to include UCHAIN.INFO native DApps</li><li>Added UCHAIN.INFO Microservices</li><li>Added Multi-chain search</li><li>Added personal asset management features</li><li>Added UCHAIN.INFO ID</li><li>Combined several elements into UI category</li><li>Deployment improvements (Docker image) moved to completed.</li><li>Update Research Phase items to 2023.</li></ul> |
